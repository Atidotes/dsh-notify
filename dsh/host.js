...
const DEFAULT_CONFIG = {
  ...
  /**
   * banner 模式的宽度**上限**（96 DPI 下的逻辑像素）：默认 300。
 ...
 * 34×34 应用图标、13px 半粗标题 + 12px 正文、22px 圆角浅色卡片、跟随系统浅色/深色外观。
 *
...
export function powershellBannerScript(options) {
   ...
   */
  const minWidth = Number.isFinite(options.minWidth) && options.minWidth > 0 ? Math.round(options.minWidth) : 200
  /** 圆角半径：越大越圆；构建 Region 时会夹到「高度的一半」以内，避免矮卡片画歪。 */
  const radius = Number.isFinite(options.radius) && options.radius > 0 ? Math.round(options.radius) : 22
  /**
