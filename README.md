...
| `bannerWidth` | `330` | 宽度**上限**（96 DPI 下的逻辑像素）：卡片会按标题/正文自己收窄，不留一截空白 |
| `bannerMinWidth` | `250` | 宽度**下限**；把上下限设成同一个值 = 钉死宽度 |
| `bannerRadius` | `24` | 圆角半径（越大越圆）；自动限制在卡片高度的一半以内 |
...
（默认 **250–330** 之间；正文右侧另留 16px 呼吸位，长提问最多 330 后换行）、**高度 = 标题 + 正文 +
底部 5px**（用 `TextRenderer.MeasureText` 量正文真实行高，单行 ≈ **49**、两行 ≈ **64**；
测量带 `NoPadding`，否则默认测量值里的边框留白会让卡片凭空高几像素）、**34×34** 官方
彩色图标（垂直居中）、**13px 半粗标题 + 12px 正文**、左内边距 10px / 右 16px、**24px 圆角**、
...
> **关于「弹出窗太大 / 留白太多 / 太长 / 太短」**：宽度贴着内容（250–330）、高度按正文行数自适应
...
| `bannerWidth` | `330` | banner 宽度上限（逻辑像素） |
| `bannerMinWidth` | `250` | banner 宽度下限 |
| `bannerRadius` | `24` | banner 圆角半径 |
