...
| `bannerWidth` | `300` | 宽度**上限**（96 DPI 下的逻辑像素）：卡片会按标题/正文自己收窄，不留一截空白 |
| `bannerMinWidth` | `200` | 宽度**下限**；把上下限设成同一个值 = 钉死宽度 |
| `bannerRadius` | `22` | 圆角半径（越大越圆）；自动限制在卡片高度的一半以内 |
...
> **关于「弹出窗太大 / 留白太多 / 太长」**：宽度贴着内容（200–300）、高度按正文行数自适应
...
| `bannerWidth` | `300` | banner 宽度上限（逻辑像素） |
| `bannerMinWidth` | `200` | banner 宽度下限 |
| `bannerRadius` | `22` | banner 圆角半径 |
