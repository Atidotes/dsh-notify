...
| `bannerMinWidth` | `290` | 宽度**下限**；把上下限设成同一个值 = 钉死宽度 |
| `bannerRadius` | `32` | 圆角半径（越大越圆）；自动夹在卡片高度的一半（= 胶囊形）以内 |
...
（默认 **290–360** 之间；正文右侧另留 16px 呼吸位，长提问最多 360 后换行）、**高度 = 标题 + 正文 +
...
> **关于「弹出窗太大 / 留白太多 / 太长 / 太短」**：宽度贴着内容（290–360）、高度按正文行数自适应
...
| `bannerMinWidth` | `290` | banner 宽度下限 |
| `bannerRadius` | `32` | banner 圆角半径（上限 = 高度一半） |
