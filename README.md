...
> 行数自适应（单行 ≈ 56），上下留白 12 / 9、图标 38px。**圆角上限 = 卡片高度的一半**，所以「更圆」
...
**数值字段**：像素/毫秒类（弹出窗宽度、最小宽度、圆角、高度、停留时长、最短运行时长、提醒间隔）
**可以填小数**；计数类（最多提醒次数、提问片段长度）**只能填整数**。超范围会就地标红并给出允许区间
（例如 `160 – 1200`），还有不合法字段时保存按钮是禁用的 —— 不会等你点了保存才回一句"失败"。
输入框是普通文本框：`-`、`12.`、`.5` 这种中间态不会被吞，失焦后不合法内容自动丢弃。
...
>
> 页面挂载时会取一次 `/dsh-notify/feed` 拿平台与文案口径，结果缓存在 `sessionStorage`
> （TTL 60 秒）：切页 / 重挂插件不会重复请求。**`backend` 不缓存** —— 通道会变，
> 它由 SSE 的状态广播或首个轮询刷新，免得旧值把「该用浏览器兜底」的判断带偏。
