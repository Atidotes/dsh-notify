 ...
 *   node scripts/windows-check.mjs --position topleft --duration 0 --width 360 --min-width 240 --radius 18 --height 0
 */
...
const width = Number(flag('width', '360'))      // 宽度上限
const minWidth = Number(flag('min-width', '240'))
const radius = Number(flag('radius', '18'))
...
const banner = powershellBannerScript({
  ...
  radius: Number.isFinite(radius) ? radius : 18,
...
自适应（240–360 之间，短消息 ≈ 240）、高度按正文行数自适应（单行 ≈ 49、两行 ≈ 64）、
34×34 官方图标、13px 标题 + 12px 正文、底部只留 5px、**18px 圆角**；8 秒自动消失，点击打开 DSH。
